...
async def test_with_real_api_data():
    ...
    print("=" * 60)
    print("Testing with Real API Data + User Statistics")
    print("=" * 60)
    ...
    print(f"  Global Rank: #{user_info.get('global_rank', 'N/A')}")
    print(f"  Country Rank: #{user_info.get('country_rank', 'N/A')}")
    print(f"  User PP: {user_info.get('pp', 'N/A')}")
    print(f"  Beatmap: {beatmap_info['title']} - {beatmap_info['version']}")
    print(f"  Score: {score_info['score']:,}")
    print(f"  Accuracy: {score_info['accuracy'] * 100:.2f}%")
    ...
    print("\nGenerating image with avatar and rankings...")
    try:
        img_bytes = await generator.generate_score_image(
            user_info, score_info, beatmap_info
        )
        
        output_path = "test_real_api_data.png"
        with open(output_path, "wb") as f:
            f.write(img_bytes.read())
        
        size_kb = len(img_bytes.getvalue()) / 1024
        print(f"\n[OK] Image generated: {output_path} ({size_kb:.1f} KB)")
        print("  - Avatar should be at (27, 532) with rounded corners")
        print("  - Global rank should be at (985, 260)")
        print("  - Country rank should be at (283, 630)")
